...
class NfcoreTemplate {
    ...
    //
    public static void email(workflow, params, summary_params, projectDir, log, ratioconv_report=[]) {
        ...
        // On success try attach the ratioconv report
        def mqc_report = null
        try {
            if (workflow.success) {
                mqc_report = ratioconv_report.getVal()
                if (mqc_report.getClass() == ArrayList && mqc_report.size() >= 1) {
                    if (mqc_report.size() > 1) {
                        log.warn "[$workflow.manifest.name] Found multiple reports from process 'RATIOCONV', will use only one"
                    }
            ...
            if (ratioconv_report) {
                log.warn "[$workflow.manifest.name] Could not attach RatioConv report to summary email"
            }
        }
        ...
        def max_ratioconv_email_size = params.max_ratioconv_email_size as nextflow.util.MemoryUnit
        def smail_fields           = [ email: email_address, subject: subject, email_txt: email_txt, email_html: email_html, projectDir: "$projectDir", mqcFile: mqc_report, mqcMaxSize: max_ratioconv_email_size.toBytes() ]
        ...
        if (email_address) {
            try {
                ...
                if ( mqc_report.size() <= max_ratioconv_email_size.toBytes() ) {
